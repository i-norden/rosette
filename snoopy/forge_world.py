"""forge-world protocol implementations for snoopy.

Implements the forge-world protocols directly in the snoopy package,
enabling benchmark-modify-benchmark cycles via ``forge bench -m snoopy.forge_world``.

SnoopyDataset is seed-aware:
...
def _severity_from_str(s: str) -> Severity:
    """Convert snoopy severity string to forge-world Severity enum."""
    mapping = {
...
def _finding_from_snoopy_dict(d: dict[str, Any]) -> Finding:
    """Convert a snoopy finding dict to a forge-world Finding."""
    return Finding(
        title=d.get("title", ""),
...
class SnoopyPipeline:
    """Snoopy's analysis pipeline as a forge-world Pipeline.
    ...
    def analyze(self, item: Any) -> list[Finding]:
        """Run snoopy analysis on an item (image path or PDF path)."""
    ...
    def _analyze_image(self, path: Path) -> list[Finding]:
        ...
        try:
            snoopy_findings = run_image_forensics(
                str(path), figure_id=path.name, config=self._config,
            )
            return [_finding_from_snoopy_dict(f) for f in snoopy_findings]
    ...
    def _analyze_pdf(self, path: Path) -> list[Finding]:
        ...
        from snoopy.demo.runner import _analyze_pdf as snoopy_analyze_pdf

        figures_dir = _PROJECT_ROOT / "data" / "figures" / "forge_world"
        figures_dir.mkdir(parents=True, exist_ok=True)

        try:
            result = snoopy_analyze_pdf(path, figures_dir)
            return [_finding_from_snoopy_dict(f) for f in result.get("findings", [])]
        except Exception as exc:
...
class SnoopyAggregator:
    """Snoopy's aggregate_findings as a forge-world Aggregator."""
...
class SnoopyDataset:
    """Snoopy's benchmark dataset as a forge-world LabeledDataset.
    ...
    def _load_fixed(self) -> list[LabeledItem]:
        ...
        if rsiil_dir.exists():
            try:
                ...
                logger.warning("Could not import snoopy fixtures module")
...
class SnoopyRules:
    """Pass/fail rules matching snoopy's demo runner logic.
...
def create_pipeline() -> SnoopyPipeline:
    return SnoopyPipeline()


def create_aggregator() -> SnoopyAggregator:
    return SnoopyAggregator()


def create_dataset() -> SnoopyDataset:
    return SnoopyDataset()


def create_rules() -> SnoopyRules:
    return SnoopyRules()
