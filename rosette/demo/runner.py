...
from rosette.analysis.cross_reference import compute_ahash, compute_phash
from rosette.analysis.evidence import aggregate_findings
from rosette.analysis.run_analysis import run_image_forensics, run_intra_paper_cross_ref
from rosette.config import AnalysisConfig
from rosette.reporting.dashboard import generate_dashboard
from rosette.reporting.pretty import (
    console,
    create_progress,
)
...
# Resolve project root: rosette/demo/runner.py -> project root
...
def _extract_and_analyze_text(pdf_path: Path) -> tuple[list[dict], dict]:
    ...
    try:
        ...
        try:
            from rosette.analysis.statistical import grim_test
        ...
        try:
            from rosette.analysis.statistical import pvalue_check
        ...
        try:
            from rosette.analysis.statistical import benford_test
        ...
        try:
            from rosette.analysis.run_analysis import run_statistical_tests
        ...
        try:
            from rosette.analysis.run_analysis import run_sprite_analysis
            from rosette.extraction.stats_extractor import extract_means_sds_and_ns
        ...
        try:
            from rosette.analysis.run_analysis import run_tortured_phrases
...
def _extract_and_analyze_tables(pdf_path: Path) -> list[dict]:
    ...
    try:
        from rosette.analysis.statistical import duplicate_value_check
        from rosette.extraction.table_extractor import extract_tables
...
async def _run_llm_analysis(
    ...
    """
    try:
        from rosette.analysis.llm_vision import analyze_figure_detailed, screen_figure
        from rosette.llm.claude import ClaudeProvider
...
def run_demo(
    ...
    from rosette.demo.fixtures import RSIIL_DATA_DIR, download_rsiil_zenodo
    ...
    if rsiil_needs_download:
        ...
        console.print()
        with httpx.Client(
            headers={"User-Agent": "rosette/0.1 (academic research tool)"},
    ...
    from rosette.demo.fixtures import _is_ground_truth, _is_pristine_ref
    ...
    try:
        from rosette.reporting.proof import generate_html_report
