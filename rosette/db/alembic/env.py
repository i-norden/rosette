"""Alembic environment configuration for rosette.

Supports both synchronous and asynchronous SQLAlchemy engines.  The database
URL is read from the rosette configuration (``RosetteConfig.storage.database_url``)
...
def _get_url() -> str:
    """Return the database URL, preferring the rosette config."""
    try:
        ...
        return config.get_main_option("sqlalchemy.url", "sqlite:///rosette.db")
